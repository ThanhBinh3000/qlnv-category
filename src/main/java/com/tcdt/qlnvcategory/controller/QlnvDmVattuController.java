...
@RestController
@RequestMapping(value = "/dmuc-vattu")
@Slf4j
@Api(tags = "Danh mục vật tư")
public class QlnvDmVattuController extends BaseController {

	@Autowired
	private QlnvDmVattuRepository qlnvDmVattuRepository;

	@ApiOperation(value = "Lấy chi tiết thông tin vật tư", response = List.class)
	@GetMapping(value = "/chi-tiet/{ids}", produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<Resp> detail(
			@ApiParam(value = "ID vật tư", example = "1", required = true) @PathVariable("ids") String ids) {
		Resp resp = new Resp();
		try {
	...
	@ApiOperation(value = "Lấy danh sách vật tư", response = List.class)
	...
	public ResponseEntity<Resp> colection(@Valid @RequestBody QlnvDmVattuSearchReq objReq) {
		...
		try {
			...
			Page<QlnvDmVattu> data = qlnvDmVattuRepository.selectParams(objReq.getMaVtu(), objReq.getTenVtu(),
	...
	@ApiOperation(value = "Lấy danh sách vật tư all", response = List.class)
	@PostMapping(value = "/danh-sach-all", produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseStatus(HttpStatus.OK)
	...
	@ApiOperation(value = "Thêm mới vật tư", response = List.class)
	...
	public ResponseEntity<Resp> create(@Valid @RequestBody QlnvDmVattuReq objReq, HttpServletRequest req) {
		Resp resp = new Resp();
		try {
			if (qlnvDmVattuRepository.findByMaVtu(objReq.getMaVtu()) != null)
				throw new UnsupportedOperationException("Mã vật tư đã tồn tại");
	...
	@ApiOperation(value = "Sửa chi tiết vật tư", response = List.class)
	...
	public ResponseEntity<Resp> edit(@Valid @RequestBody QlnvDmVattuReq objReq, HttpServletRequest req) {
		...
		try {
			...
			if (!qOptional.isPresent())
				throw new UnsupportedOperationException("Mã vật tư không tồn tại");
	...
	@ApiOperation(value = "Xóa vật tư", response = List.class)
