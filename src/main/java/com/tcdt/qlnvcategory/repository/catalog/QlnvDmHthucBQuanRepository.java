...
public interface QlnvDmHthucBQuanRepository extends CrudRepository<QlnvDmHthucBQuan, Long> {
	@Query(value = "SELECT * FROM QLNV_DM_HTHUC_BQUAN t WHERE (:maHthuc is null or lower(t.MA_HTHUC) like lower(concat(concat('%', :maHthuc),'%'))) AND (:tenHthuc is null or lower(t.TEN_HTHUC) like lower(concat(concat('%', :tenHthuc),'%'))) "
			+ " AND (:trangThai is null or t.TRANG_THAI = :trangThai)", 
			countQuery = "SELECT count(1) FROM QLNV_DM_HTHUC_BQUAN t "
