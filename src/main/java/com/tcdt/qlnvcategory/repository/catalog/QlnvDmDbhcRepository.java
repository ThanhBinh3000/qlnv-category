...
public interface QlnvDmDbhcRepository extends CrudRepository<QlnvDmDbhc, Long> {
	...
	@Transactional
	@Modifying
	@Query(value = "DELETE FROM DM_DBHC u WHERE u.ID in ?1", nativeQuery = true)
	int deleteWithIds(List<Long> items);

	QlnvDmDbhc findByMaDbhc(String maDbhc);

	@Query(value = "SELECT * FROM DM_DBHC t WHERE (:maDbhc is null or lower(MA_DBHC) like lower(concat(concat('%', :maDbhc),'%'))) "
			+ "AND (:tenDbhc is null or lower(TEN_DBHC) like lower(concat(concat('%', :tenDbhc),'%'))) AND (:trangThai is null or t.TRANG_THAI = :trangThai)", countQuery = "SELECT count(1) FROM DM_DBHC t "
	...
	@Query(value = "SELECT * FROM DM_DBHC WHERE (:maCha is null or MA_CHA = :maCha) AND TRANG_THAI = :hoatDong order by id", nativeQuery = true)
