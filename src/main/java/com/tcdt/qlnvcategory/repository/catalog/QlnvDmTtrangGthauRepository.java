...
public interface QlnvDmTtrangGthauRepository extends CrudRepository<QlnvDmTtrangGthau, Long> {
	@Query(value = "SELECT * FROM DM_TTRANG_THAU t WHERE (:maTtrang is null or lower(t.MA_TTRANG) like lower(concat(concat('%', :maTtrang),'%'))) "
			+ "AND (:tenTtrang is null or lower(t.TEN_TTRANG) like lower(concat(concat('%', :tenTtrang),'%'))) AND (:trangThai is null or t.TRANG_THAI = :trangThai)", countQuery = "SELECT count(1) FROM DM_TTRANG_THAU t "
