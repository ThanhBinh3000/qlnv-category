package com.tcdt.qlnvcategory.request.object.catalog;

import javax.validation.constraints.NotNull;
import javax.validation.constraints.Size;

import com.tcdt.qlnvcategory.util.Contains;

import io.swagger.annotations.ApiModelProperty;
import lombok.Data;

@Data
public class QlnvDmTchuanDtlReq {
	@ApiModelProperty(notes = "Bắt buộc set đối với update")
	Long id;

	@NotNull(message = "Không được để trống")
	@Size(max = 20, message = "Mã tiêu chuẩn không được vượt quá 20 ký tự")
	@ApiModelProperty(example = "MATC001")
	String maTchuan;

	@NotNull(message = "Không được để trống")
	@Size(max = 250, message = "Tên tiêu chuẩn không được vượt quá 20 ký tự")
	@ApiModelProperty(example = "Tên tiêu chuẩn")
	String tenTchuan;

	@NotNull(message = "Không được để trống")
	@Size(max = 250, message = "Tên hàng không được vượt quá 250 ký tự")
	@ApiModelProperty(example = "Chỉ số")
	String chiSo;

	@NotNull(message = "Không được để trống")
	@Size(max = 20, message = "Phương pháp không được vượt quá 20 ký tự")
	String phuongPhap;

	Long tcHdrId;

	@NotNull(message = "Không được để trống")
	@Size(max = 2, message = "Trạng thái không được vượt quá 2 ký tự")
	@ApiModelProperty(example = Contains.HOAT_DONG)
	String trangThai;
}
